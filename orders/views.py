...
def create_order(request):
    ...
    total_amount = sum(item.quantity * item.product_listing.selling_price for item in cart_items)
    ...
    for item in cart_items:
        OrderDetails.objects.create(
            order=order,
            order_item=item.product_listing,
            quantity=item.quantity,
            price=item.product_listing.selling_price * item.quantity
        )
